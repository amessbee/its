...
def add_author_to_questions(input_file, output_file, author_name='Dr. Umar Suleman', topic_name1=None):
    ...
    for i, question_group_elem in enumerate(input_root.iter('question_group')):
        ...
        # Assign topic_name 
        if i is not None and topic_name1 is not None:
        ...
        if question_topic_elem is not None:
            question_elem.remove(question_topic_elem)
        ...
        for question_elem in question_group_elem.findall('question'):
            process_question_elem(question_elem, output_question_group_elem, output_tree, question_number, author_name, topic_name1, i)
            question_number += 1
    ...
    for i, question_elem in enumerate(input_root.iter('question')):
        process_question_elem(question_elem, output_root, output_tree, question_number, author_name, topic_name1, i)
        question_number += 1

    # Save the modified output XML file
    output_tree.write(output_file)

def process_question_elem(question_elem, output_parent_elem, output_tree, question_number, author_name, topic_name1, i):
...
def process_files_in_directory(directory, xml_file = 'bank.xml'):
    ...
    for filename in files:
        ...
        for i in range(4,len(parts) ):
            topic_name = topic_name + ' ' + parts[i]
        print(filename)
        #topic_name = os.path.splitext(topic_name)[0]
        # Call add_author_to_questions with the extracted values
        input_file = os.path.join(directory, filename)
        output_file = xml_file
        add_author_to_questions(input_file, output_file, author_name, topic_name)
