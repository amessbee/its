...
total_number_of_questions = 75
number_of_analytical_questions = 15
...
def move_question_groups(input_file, output_file, topic_name = 'Analytical', unselected_file = None,):
    ...
    # Randomly select up to two question groups
    selected_groups = random.sample(question_groups, min(2, len(question_groups)))
    ...
    remaining_count = total_number_of_questions - number_of_analytical_questions

    if remaining_count > 0:
        # Randomly select additional questions from different question groups
        #selected_additional_groups = random.sample(other_question_groups, min(remaining_count, len(other_question_groups)))
        selected_additional_groups = random.sample(other_question_groups, min(2, len(other_question_groups)))
